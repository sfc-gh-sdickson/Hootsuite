...
CREATE OR REPLACE AGENT HOOTSUITE_INTELLIGENCE_AGENT
  ...
  instructions:
    ...
    orchestration: "For campaign performance questions use CampaignAnalyst. For customer health and support analysis use CustomerHealthAnalyst. For social media performance, posts, and engagement use SocialPerformanceAnalyst. For support tickets search use SupportTicketSearch. For help articles search use KnowledgeBaseSearch. For marketing assets search use MarketingAssetSearch. For ML predictions use the appropriate prediction function. To trigger customer engagement workflows use TriggerCustomerEngagement. When users want to create an interactive app or deeper analysis use GenerateStreamlitApp with the SQL query, use exploratory analysis type unless user specifies statistical."
    system: "You are an expert social media intelligence agent for Hootsuite. You help analyze campaign performance, customer health, engagement metrics, and support operations. You can automate customer success workflows and generate interactive Streamlit applications from any query. Always provide data-driven insights based on available data."
    sample_questions:
      ...
      - question: "Create a Streamlit app to analyze engagement rates by platform"
        answer: "I'll use GenerateStreamlitApp with a query for engagement data by platform, using exploratory analysis type to create an interactive dashboard."
  ...
  tools:
    ...
    - tool_spec:
        ...
        description: "Generates and deploys an interactive Streamlit application from chart data, returning a clickable link. Use when users want to create an app, build a dashboard, or do deeper interactive analysis. Always use exploratory unless user specifies statistical or time_series."
        input_schema:
          type: "object"
          properties:
            CHART_SQL:
